...
IMAGE_S390X = $(REGISTRY)nfs-client-provisioner-s390x:$(VERSION) 
MUTABLE_IMAGE = $(REGISTRY)nfs-client-provisioner:latest
MUTABLE_IMAGE_ARM = $(REGISTRY)nfs-client-provisioner-arm:latest
MUTABLE_IMAGE_S390X = $(REGISTRY)nfs-client-provisioner-s390x:latest
...
build_s390x:
	CGO_ENABLED=0 GOOS=linux go build -a -ldflags '-extldflags "-static"' -o docker/s390x/nfs-client-provisioner ./cmd/nfs-client-provisioner
...
image_s390x:
	docker build -t $(MUTABLE_IMAGE_S390X) docker/s390x 
	docker tag $(MUTABLE_IMAGE_S390X) $(IMAGE_S390X)
...
push:
	...
	docker push $(IMAGE_S390X)
	docker push $(MUTABLE_IMAGE_S390X)
